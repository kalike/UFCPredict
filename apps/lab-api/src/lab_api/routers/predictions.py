...
def _reconstruct_session_fights(db: Session, ds, session_id: int) -> list[RichFightPrediction]:
    ...
    for fight_id, plist in by_fight.items():
        ...
        items.append((f.fight_order if f.fight_order is not None else 9999, RichFightPrediction(
            ...
            real_winner=_fight_real_winner(name1, name2, f.result, f.real_winner),
            fight_id=f.id,
...
def _build_session_summary(db: Session, ds, s) -> SessionSummary:
    """Build the list-row summary for a single PredictionSession."""
...
def list_sessions(db: Session = Depends(get_db)) -> list[SessionSummary]:
    ...
    return [_build_session_summary(db, ds, s) for s in sessions]


@router.get("/sessions/realworld", response_model=list[SessionSummary])
def list_realworld_sessions(db: Session = Depends(get_db)) -> list[SessionSummary]:
    """Sessions auto-generated by the recalculation service over the RealWorld
    held-out window. Mirrors exactly what the dashboard evaluates: the most recent
    lab_recalc session per real UFC event with event_date >= REALWORLD_CUTOFF.
    Detail is served by the shared GET /sessions/{id} (source-agnostic)."""
    from lab_api.services.dashboard import _realworld_sessions_by_event
    ds = get_data_store()
    by_event = _realworld_sessions_by_event(db)
    out = [_build_session_summary(db, ds, s) for s in by_event.values()]
    out.sort(key=lambda s: s.event_date or "", reverse=True)
    return out
